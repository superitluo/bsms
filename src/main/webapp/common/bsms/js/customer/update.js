layui.use(['jquery', 'layer', 'form', 'laydate'], function () {
    ...
    $(document).ready(function () {
        ...
        $.get('../customer/customerById', {id: id}, function (customer) {
            $('input[name="name"]').val(customer.name);
            $('input[name="cardnumber"]').val(customer.cardnumber);
            $(":radio[name='gender'][value='" + customer.gender + "']").prop("checked", true);
            form.render();
            $('input[name="birthday"]').val(customer.birthday);
            $('input[name="phone"]').val(customer.phone);
            $('input[name="address"]').val(customer.address);
            $('select[name="cardtypeid"]').val(customer.cardtypeid);
            form.render('select');
        });
    });
    //表单提交按钮
    form.on('submit(save)', function (data) {
        ...
        $.ajax({
            ...
            data: {
                id: id, name: name, cardnumber: cardnumber, gender: gender, birthday: birthday
                , phone: phone, address: address, cardtypeid: cardtypeid
            },
            dataType: "json",
            success: function (result) {
                parent.layer.alert('修改会员成功', {offset: 't', icon: 1, time: 2000});
                parent.layer.close(index); //关闭当前弹窗
                parent.$('#customer-datagrid-filter').datagrid('refresh', false);
            }
            , error: function () {
                parent.layer.alert('修改会员失败', {offset: 't', icon: 5, time: 2000});
            }
        });
