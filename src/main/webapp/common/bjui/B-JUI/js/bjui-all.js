    ...
    var BJUI = {
        ...
        initLayout: function () {
            var ww = $(window).width(), hh = $(window).height(),
                $top = $('#bjui-top'), $navbar = $('#bjui-navbar'),
                $collapse = $('#bjui-top-collapse, #bjui-navbar-collapse'),
                ...
                $sidenavarrow = $('#bjui-sidenav-arrow'), $sidenavbtn = $('#bjui-sidenav-btn'),
                $sidenavbox = $('#bjui-sidenav-box'),
            ...
            $collapse.each(function () {
                var $this = $(this), $collapsebtn = $this.prev().find('.navbar-toggle'),
                    isCollapse = ($this.scrollTop() == 5 || $this.hasClass('navbar-show'))
...
+function ($) {
    ...
    Basedrag.prototype.start = function (e) {
        ...
        $(document)
            .on('mouseup.bjui.basedrag', function (e) {
                that.stop.apply(that, [e])
            })
            .on('mousemove.bjui.basedrag', function (e) {
    ...
    Basedrag.prototype.stop = function (e) {
        ...
        if (that.options.drop && !that.isDrop) {
            ...
            that.$placeholder && that.$placeholder.animate({
                top: offset.top,
                left: offset.left,
                opacity: 0.2
            }, 'normal', function () {
...
+function ($) {
    ...
    Sidenav.prototype.ajaxHnav = function () {
        var that = this, options = that.options, childKey = options.childKey || 'children',
            $li = that.$element.parent(), $box = $('#bjui-sidenav-box'), html = BJUI.StrBuilder(),
    ...
    Sidenav.prototype.initHnav = function () {
        var that = this, options = that.options, childKey = options.childKey || 'children',
            targetKey = options.targetKey || 'target', idKey = options.idKey || 'id',
...
+function ($) {
    ...
    var currentIndex, $currentTab, $currentPanel, $box, $tabs, $panels, $prevBtn, $nextBtn, $moreBtn, $moreBox, $main,
        $mainLi,
        autorefreshTimer
    ...
    $(function () {
        var INIT_NAVTAB = function () {
            ...
            $moreBtn.click(function () {
                ...
                var hh = $(window).height(), mh = $moreBox.height(),
                    th = $('#bjui-top').height() + $('#bjui-navbar').height(), sh = hh - th - 26
            ...
            var mainTit, options = $.extend({}, Navtab.DEFAULTS, $main.data(), {id: 'main', title: mainTit}),
                mainTab = new Navtab(options)
            ...
            $main
                ...
                .click(function () {
                    if (!$(this).hasClass('active')) BJUI.navtab('switchTab', 'main')
                })
        ...
        var tools = {
            ...
            switchTab: function (iTabIndex) {
                var $tab = this.getTabs().removeClass('active').eq(iTabIndex).addClass('active'),
                    $panels = this.getPanels(), $panel = $panels.eq(iTabIndex),
                    onSwitch = that.options.onSwitch ? that.options.onSwitch.toFunc() : null
...
+function ($) {
    ...
    Dialog.prototype.TOOLS = function () {
        ...
        var tools = {
            ...
            reload: function ($dialog, options) {
                var tools = this, $dialogContent = $dialog.find('> .dialogContent'), onLoad,
                    data = options && options.data, html
    ...
    Dialog.prototype.reload = function (option) {
        ...
        if ($dialog && $dialog.length) {
            ...
            var _reload = function () {
                var $dialogContent = $dialog.find('> .dialogContent'), ww = $(window).width(), hh = $(window).height(),
                ...
                if (w != op.width) {
                    ...
                    if (options.max) {
                        $dialog.animate({
                            width: w,
                            left: (ww - w) / 2
                        }, 'normal'/*, function() { $dialogContent.width(options.width) }*/)
                    } else {
...
+function ($) {
    ...
    Bjuiajax.prototype.ajaxerror = function (xhr, ajaxOptions, thrownError) {
        var msg = xhr.responseText, that = this, options = that.$element.data('bjui.ajax.options') || that.options,
    ...
    Bjuiajax.prototype.ajaxcallback = function (json) {
        var that = this, options = that.options,
            okCallback = options.okCallback, errCallback = options.errCallback,
            okAfterCallback = options.okAfterCallback,
        ...
        if (json[BJUI.keys.statusCode] == BJUI.statusCode.ok) {
            ...
            if (tabids.length) {
                $.unique(tabids)
                setTimeout(function () {
                    BJUI.navtab('reloadFlag', tabids.join(','))
                }, 100)
            }
            if (dialogids.length) {
                $.unique(dialogids)
                setTimeout(function () {
                    BJUI.dialog('refresh', dialogids.join(','))
                }, 100)
            }
            if (divids.length) {
                $.unique(divids)
                setTimeout(function () {
                    that.refreshdiv(divids.join(','))
                }, 100)
            }
            if (datagrids.length) {
                setTimeout(function () {
    ...
    Bjuiajax.prototype._ajaxform = function ($form, options) {
        ...
        if ($uploaders.length && WebUploader) {
            $uploaders.each(function (i, n) {
                var uploader = $(this).data('webuploader'),
                    $lis = $(this).data('webuploader.wrap').find('.filelist > li'), fileCount = $lis.length,
                    waitCount = fileCount - $lis.filter('.uploaded').length
                ...
                callback: function (html) {
                    ...
                    if (arefre)
                        autorefreshTimer = setInterval(function () {
                            $target.bjuiajax('refreshlayout', $target)
                        }, arefre * 1000)
                    if (!$target.height()) {
    ...
    Alertmsg.prototype.TOOLS = function () {
        ...
        var tools = {
            ...
            openPosition: function () {
                var position = BJUI.alertMsg.displayPosition, mode = BJUI.alertMsg.displayMode, width = 460,
                    height = $box.outerHeight(), startCss = {}, endCss = {}
...
+function ($) {
    ...
    Pagination.prototype.init = function () {
        var that = this, tools = that.tools, options = that.options, pr = BJUI.regional.pagination,
            btnpaging = FRAG.gridPaging, pageNums = [], interval, selectPages = [], pagingHtml = BJUI.StrBuilder()
        ...
        that.$element.on('click.bjui.pagination.pagenum', 'li.page-num', function (e) {
            ...
            var pageCurrent = that.options.pageCurrent,
                interval = tools.getPageInterval(that.pageCount, pageCurrent, options.showPagenum), pageNums = []
...
+function ($) {
    ...
    var $box, $main, $prev, $next, $year, $month, $time, $timeinps, $spinner, $hh, $mm, $ss, $tm, $close, $days,
        $dayNames, $clearBtn, $okBtn
    ...
    Datepicker.prototype.TOOLS = function () {
        ...
        var tools = {
            ...
            changeTm: function ($input, $btn) {
                var ivalue = parseInt($input.val()), istart = parseInt($input.data('start')) || 0,
                    iend = parseInt($input.data('end'))
...
+function ($) {
    ...
    Datagrid.prototype.TOOLS = function () {
        ...
        var tools = {
            ...
            getRight4Lock: function (index) {
                var width = 0, $td = that.$lockTbody.find('> tr:first > td:eq(' + index + ')'),
                    $firstTds = $td && $td.prevAll().add($td)
            ...
            afterSave: function ($trs, data) {
                var afterSave = options.afterSave, childUpdate = that.options.childUpdate,
                    $parent = that.$element.data('bjui.datagrid.parent'),
            ...
            json2Array4Tree: function (options, datas, level, pid) {
                ...
                var k = options.key, pk = options.parentKey, childKey = options.childKey, childLen = options.childLen,
            ...
            array2Json4Tree: function (options, datas) {
                var r = [], temp = [], keys = options.keys, key = keys.key, parentKey = keys.parentKey,
            ...
            initTbody: function (data, refreshFlag) {
                var tools = this, allData = that.allData, type = options.dataType || 'json', model = that.columnModel,
                    hiddenFields = that.hiddenFields, regional = that.regional, newData = [], attach = that.attach, json
            ...
            createTrs: function (datas, refreshFlag) {
                var tools = this, keys = that.options.keys, model = that.columnModel, paging = that.paging, trs = [],
                    editFrag = BJUI.doRegional(FRAG.gridEditBtn, that.regional),
                    childFrag = BJUI.doRegional(FRAG.gridExpandBtn, that.regional), lockedCols = [],
                    isRenderOnly = false,
                ...
                for (var i = 0, l = datas.length; i < l; i++) {
                    var trData = datas[i], modellength = model.length, tempcolspan = modellength,
                        linenumber = options.linenumberAll ? ((paging.pageCurrent - 1) * paging.pageSize + (i + 1)) : (i + 1),
                        tds = [], n, tdHtml = BJUI.StrBuilder(), $td, name, label, _label, render_label, align, cls,
                        display, tree, hasTree = false, treeattr = '', tdTemplate = options.tdTemplate
            ...
            showSubMenu: function ($li, $menu, $submenu) {
                var left, width = $menu.outerWidth(), submenu_width = $submenu.data('width') || $submenu.outerWidth(),
                    wh = that.$grid.height(), mt, submenu_height = $submenu.data('height'), animate_op,
                    boxWidth = that.$boxH.width()
            ...
            locking: function ($th) {
                var index = $th.data('index'), columnModel = that.columnModel[index], lockFlag = columnModel.lock,
                    locked = columnModel.locked, $menu = that.$menu, $ul = $menu.find('> ul'),
                    $lockli = $ul.find('> li.' + that.classnames.li_lock), $unlockli = $lockli.next()
                    ...
                    $.each(that.columnModel, function (i, n) {
                        ...
                        var colClick = function (n) {
                            $col.click(function () {
                                ...
                                var $this = $(this), check = !$this.find('i').hasClass('fa-check-square-o'),
                                    index = n.index
            ...
            showhide: function (model, showFlag) {
                var keys = that.options.keys, index = model.index, $th = model.th, $trs = that.$tbody.find('> tr'),
            ...
            quickSort: function (model) {
                ...
                var $th = model.th, data = that.data, allData = that.allData, postData, direction, name = model.name,
                    type = model.type, $ths = that.$thead.find('> tr > th.datagrid-quicksort-th')
                ...
                var sortData = function (data, orders) {
                    var localSort = function (a, b) {
                        ...
                        function doCompare() {
                            var keys = orders[key_index].split(' '), name = keys[0], direction = keys[1],
                                typeA = (typeof a[name]), typeB = (typeof b[name])
            ...
            showFilterMsg: function (msgs) {
                var $msg = that.$boxH.find('.datagrid-thead-dialog-filter-msg'), $filter = $msg.find('> .msg-filter'),
                    html = BJUI.StrBuilder()
            ...
            quickFilter: function (model, filterDatas) {
                ...
                var tools = this, $th = model.th, data = that.data, allData = that.allData, name = model.name, postData,
                    fDatas, msgs = []
            ...
            setDomData: function (tr) {
                var columnModel = that.columnModel, data = {}, hideDatas = tr.attr('data-hidden-datas'),
                    attach = that.attach
            ...
            updateGridIndex: function () {
                var paging = that.paging, options = that.options,
                    startNumber = (paging.pageCurrent - 1) * paging.pageSize
    ...
    Datagrid.prototype.init = function () {
        ...
        var that = this, options = that.options, keys = options.keys, tools = that.tools,
            $parent = that.$element.parent(), gridHtml = BJUI.StrBuilder()
    ...
    Datagrid.prototype.setColumnModel = function () {
        var that = this, options = that.options, $trs = that.$thead.find('> tr'), rows = [], ths = [],
            trLen = $trs.length
        ...
        if ($trs.length && trLen == 1) {
            $trs.find('> th').each(function (i) {
                var $th = $(this).addClass('single-row').data('index', i), op = $th.data('options'),
                    oW = $th.attr('width') || 'auto', label = $th.html()
            ...
            $trs.each(function (len) {
                ...
                $(this).find('> th').each(function (i) {
                    var $th = $(this), op = $th.data('options') || $th.data('datagrid.column') || {},
                        colspan = parseInt(($th.attr('colspan') || 0), 10),
                        rowspan = parseInt(($th.attr('rowspan') || 0), 10), oW = $th.attr('width') || 'auto',
                        label = $th.html()
                    ...
                    if (colspan) {
                        op.colspan = colspan
                        for (var start_index = (next_rows.length ? next_rows[index] : index),
                                 k = start_index; k < (start_index + colspan); k++) {
                            rows[next_index++] = k
                            ths[next_index - 1] = op
                        }
                        index += (colspan - 1)
    ...
    Datagrid.prototype.createThead = function () {
        var that = this, options = that.options, keys = options.keys, columns = options.columns.concat(), rowArr = [],
            rows = [], label, align, width, colspan, rowspan, resize, menu,
    ...
    Datagrid.prototype.refresh = function (filterFlag) {
        var that = this, options = that.options, tools = that.tools, isDom = that.isDom, pageInfo = BJUI.pageInfo,
            paging = that.paging, postData = {}
    ...
    Datagrid.prototype.showChild = function (row, flag, func) {
        ...
        var that = this, options = that.options, $child, $td, $table, index, trData, childOptions, dataUrl,
            $trs = that.$tbody.find('> tr:not(.' + that.classnames.tr_child + ')'),
            fixedH = function () {
                //if (!options.fullGrid)
                that.fixedWidth()
    ...
    Datagrid.prototype.initTop = function () {
        var that = this, options = that.options, regional = that.regional, hastoolbaritem = false, $group,
            groupHtml = '<div class="btn-group" role="group"></div>',
            btnHtml = '<button type="button" class="btn" data-icon=""></button>'
    ...
    Datagrid.prototype.initThead = function () {
        ...
        $ths.filter('.datagrid-column-menu')
            .on('filter.bjui.datagrid.th', function (e, flag) {
                var $th = $(this),
                    $btn = $th.find('> div > .' + that.classnames.th_cell + '> .' + that.classnames.btn_menu + '> .btn')
        ...
        var delayFunc = function () {
            ...
            if (that.renderTds && that.renderTds.length) {
                var $trs = that.$tbody.find('> tr:not(.' + that.classnames.tr_child + ', .datagrid-nodata)'),
                    j = that.renderTds && that.renderTds.length, fixedWidthModel = {}
                var $tempTrs = $(), tempDatas = []

                for (var i = 0; i < j; i++) {
                    var obj = that.renderTds[i],
                        label = obj.render.call(that, obj.label, obj.data, that.columnModel[obj.tdindex].items, that.columnModel[obj.tdindex].itemattr)
    ...
    Datagrid.prototype.fixedWidth = function (isInit) {
        var that = this, options = that.options, bW, excludeW = 0, fixedW, columnModel = that.columnModel,
            length = columnModel.length
        ...
        var setNewWidth = function () {
            ...
            if (options.fullGrid || that.isTemplate || that.needfixedWidth || (that.hasAutoCol && isInit)) {
                ...
                $tableB.find('> colgroup > col').each(function (i) {
                    var $col = $(this),
                        dataLength = that.data.length ? String(that.data[that.data.length - 1].gridNumber).length : 2
    ...
    Datagrid.prototype.initTbody = function () {
        var that = this, options = that.options, tools = that.tools, $trs = that.$tbody.find('> tr'),
            $tds = $trs.find('> td'), width = options.tableWidth || '0'
    ...
    Datagrid.prototype.initEvents = function ($trs) {
        ...
        $trs.on('click.bjui.datagrid.tr', function (e, checkbox) {
            var $tr = $(this), index = $tr.index(), data,
                $selectedTrs = that.$tbody.find('> tr.' + that.classnames.tr_selected), $last = that.$lastSelect,
                checked, $lockTrs = that.$lockTbody && that.$lockTbody.find('> tr')
            ...
            .on('delete.bjui.datagrid.tr', function (e) {
                ...
                var $tr = $(this), tr_index = $tr.index(), data_index = tr_index, data = that.data, gridIndex,
                    allData = that.allData, $lockTrs = that.$lockTbody && that.$lockTbody.find('> tr')
        ...
        // custom addChild
            ...
            var $this = $(this), $tr = $this.closest('tr'), $table = $tr.data('bjui.datagrid.child'),
                $child = $tr.next('.' + that.classnames.tr_child), trData, data = $this.data('addData'), addData,
        ...
        // tree expand || collapse
            ...
            var $t = $(this), $tr = $t.closest('tr'), childLen = $tr.data('child'), level = $tr.data('level'),
                isExpand = !$t.hasClass('collapsed')
        ...
        if (options.dropOptions.drop) {
            var $trs = that.$tbody.find('> tr:not(.' + that.classnames.tr_child + ')'),
                beforeDrag = options.dropOptions.beforeDrag
            ...
            $trs
                ...
                var $this = $(this).addClass('datagrid-hover'), height = $this.outerHeight() / 2,
                    top = $this.offset().top
                ...
                var $this = $(this), position, beforeDrop = options.dropOptions.beforeDrop,
                    afterDrop = options.dropOptions.afterDrop
                ...
                if (afterDrop && that.afterDrop) {
                    if (typeof afterDrop === 'function') {
                        ...
                        var postData = {pageSize: that.paging.pageSize, pageCurrent: that.paging.pageCurrent},
                            ...
                            opts = {url: options.dropOptions.dropUrl, type: 'POST', reload: false}
    ...
    Datagrid.prototype.appendTr = function (sTr, dTr, option) {
        ...
        var that = this, options = that.options, sData, dData, sIndex = that.tools.getNoChildDataIndex(sTr.index()),
            dIndex = that.tools.getNoChildDataIndex(dTr.index()),
            hasChild = sTr.next().hasClass(that.classnames.tr_child),
            keys = options.treeOptions.keys,
            sParent, sParentData, sLevel = sTr.data('level'), dLevel = (dTr.data('level') + 1) || 0,
            dChildLen = dTr.data('child') + 1,
        ...
        sTr.filter(':not(.' + that.classnames.tr_child + ')').each(function () {
            var $tr = $(this), level = $tr.data('level'), newLevel = level + (dLevel - sLevel),
                $td = $tr.find('> td.datagrid-tree-td'), label = $td.find('.datagrid-tree-title').html(),
        ...
        if (dParent && dParent.length) {
            var _childs = that.getChildrens(dParent, null, false),
                dParentData = that.data[that.tools.getNoChildDataIndex(dParent.index())]
    ...
    Datagrid.prototype.insertTr = function (sTr, dTr, position) {
        var that = this, point, options = that.options, keys = options.treeOptions.keys, sData, dData, moveIndex,
            sIndex = that.tools.getNoChildDataIndex(sTr.index()), dIndex = that.tools.getNoChildDataIndex(dTr.index()),
            hasChild = sTr.next().hasClass(that.classnames.tr_child),
            _start, _end
        var beforeDrop = options.dropOptions.beforeDrop
        ...
        if (that.isDom) {
            ...
            if (options.isTree) {
                var len, option = {}, append = false, sParent = that.getParents(sTr.first(), null, true),
    ...
    Datagrid.prototype.expand = function (rows, expandFlag) {
        var that = this, options = that.options, $trs = that.$tbody.find('> tr:not(.' + that.classnames.tr_child + ')'),
            event
    ...
    Datagrid.prototype.fixedLockItem = function (type) {
        var that = this, columnModel = that.columnModel, $filterThs = that.$thead.find('> tr.datagrid-filter > th'),
            $lockTrs = that.$lockTbody && that.$lockTbody.find('> tr')
        ...
        if (!type) {
            ...
            if (that.$lockThead) {
                ...
                if ($.inArray(that.checkboxColumn, columnModel) != -1 && that.checkboxColumn.locked) {
                    var $lockTh = that.$lockThead.find('> tr:first > th.' + that.classnames.td_checkbox),
                        index = that.checkboxColumn.index, $th = that.$thead.find('> tr:first > th:eq(' + index + ')')
    ...
    Datagrid.prototype.doLock = function () {
        var that = this, options = that.options, tools = that.tools, columnModel = that.columnModel,
            tableW = that.$tableH.width(), width = 0, $trs, $lockTrs, lockedLen = 0
        ...
        var lockH = that.$lockTableH.height(), H = that.$thead.height(),
            lockFH = that.$lockTableF && that.$lockTableF.height(), HF = that.$tableF && that.$tableF.height()
            ...
            if (lockBH != HB) {
                if (lockBH > HB) {
                    $lockTrs.each(function (tr_index) {
                        var $lockTr = $(this), $lockTd = $lockTr.find('> td:visible:first'),
                            newH = $lockTd.outerHeight()
    ...
    Datagrid.prototype.filterInThead = function (isDialog) {
        var that = this, options = that.options, tools = that.tools, regional = that.regional,
            columnModel = that.columnModel, filterData = {}
    ...
    Datagrid.prototype.showLinenumber = function (flag) {
        var that = this, options = that.options, model = that.columnModel, numberModel, modelOrder = -1,
            data = that.data, numberModel_index = $.inArray(that.linenumberColumn, model)
        ...
        if (numberModel) {
            ...
            var $trsH = that.$thead.find('> tr'), col = '<col style="width:30px;">', $th,
                $filterTr = $trsH.filter('.datagrid-filter'), rowspan = $trsH.length - $filterTr.length
    ...
    Datagrid.prototype.showCheckboxcol = function (flag) {
        var that = this, options = that.options, model = that.columnModel, numberModel = model[0], checkModel,
            modelOrder = -1, checkModel_index = $.inArray(that.checkboxColumn, model)
        ...
        if (checkModel) {
            ...
            var $trsH = that.$thead.find('> tr'), col = '<col style="width:30px;">', $th, $td,
                $filterTr = $trsH.filter('.datagrid-filter'), rowspan = $trsH.length - $filterTr.length
    ...
    Datagrid.prototype.showEditCol = function (flag) {
        var that = this, options = that.options, model = that.columnModel, editModel = model[model.length - 1],
            data = that.data

        if (editModel === that.editBtnsColumn) {
            that.showhideColumn(editModel.th, flag ? true : false)
        } else if (flag) {
            var $trsH = that.$thead.find('> tr'), col = '<col style="width:110px;">', $th, $td,
                $filterTr = $trsH.filter('.datagrid-filter'), rowspan = $trsH.length - $filterTr.length
    ...
    Datagrid.prototype.colResize = function () {
        ...
        $thead.find('> tr > th').each(function (i) {
            var $th = $(this),
                $resize = $th.find('> div > .' + that.classnames.th_cell + '> .' + that.classnames.th_resizemark)

            $resize.on('mousedown.bjui.datagrid.resize', function (e) {
                var ofLeft = that.$boxH.scrollLeft(), marginLeft = parseInt(that.$boxH.css('marginLeft') || 0, 10),
                    left, index = $th.data('index'), model = columnModel[index], width = model.th.width()
                    , $trs = that.$tbody.find('> tr'), $lockTrs = that.$lockTbody && that.$lockTbody.find('> tr'),
                    lockH = that.$lockTableB && that.$lockTableB.height(), H = that.$tableB.height(), lockH_new, H_new
                                ...
                                if (that.$lockTbody) {
                                    ...
                                    if (lockH_new != lockH || H_new != H) {
                                        if (lockH_new > H_new) {
                                            $lockTrs.each(function (tr_index) {
                                                var $lockTr = $(this),
                                                    $lockTd = $lockTr.find('> td:eq(' + model.lockIndex + ')'),
                                                    newH = $lockTd.outerHeight()
                                            ...
                                            $trs.each(function (tr_index) {
                                                var $tr = $(this), $td = $tr.find('> td:eq(' + index + ')'),
                                                    newH = $td.outerHeight()
    ...
    Datagrid.prototype.colMenu = function () {
        var that = this, options = that.options, tools = that.tools, regional = that.regional,
            $ths = that.$trsH.find('> th.' + that.classnames.th_menu),
            $menu = that.$grid.find('> .' + that.classnames.s_menu)
        ...
        $ths.each(function () {
            ...
            if (!$btnBox.length) {
                ...
                $btn.click(function () {
                    var $tr = $th.closest('tr'), rowspan = parseInt(($th.attr('rowspan') || 1), 10),
                        left = $(this).offset().left - that.$grid.offset().left - 1,
                        top = (that.$trsH.length - rowspan) * 25 + (13 * rowspan) + 11,
                        $showhide = $menu.find('> ul > li.datagrid-li-showhide'), menu_width, submenu_width
    ...
    Datagrid.prototype.colShowhide = function (showFlag) {
        var that = this, options = that.options, tools = that.tools, $menu = that.$menu, $ul = $menu.find('> ul'),
            $showhideli = $ul.find('> li.' + that.classnames.li_showhide)
    ...
    Datagrid.prototype.colFilter = function ($th, filterFlag) {
        var that = this, options = that.options, tools = that.tools, regional = that.regional,
            $filter = $th.data('bjui.datagrid.filter'), $menu = that.$menu,
            $filterli = $menu.find('> ul > li.' + that.classnames.li_filter)
        ...
        if (model.quickfilter) {
            ...
            if (!$filter || !$filter.length) {
                ...
                var index = $th.data('index'), model = that.columnModel[index], type = model.type || 'string',
                ...
                $ok.click(function () {
                    var operatorA = $selA.val(), valA = $valA.val().trim(), operatorB = $selB.val(),
                        valB = $valB.val().trim(), andor = $andOr.val()
                    var filterDatas = {}
    ...
    Datagrid.prototype.initPaging = function () {
        var that = this, tools = that.tools, options = that.options, paging = that.paging,
            pr = BJUI.regional.pagination, btnpaging = FRAG.gridPaging, pageNums = [], pageCount = paging.pageCount,
            interval, selectPages = [], pagingHtml = BJUI.StrBuilder()
        ...
        that.$boxP.on('click.datagrid.pagenum', 'li.page-num', function (e) {
            ...
            var pageCurrent = that.paging.pageCurrent,
                interval = tools.getPageInterval(that.paging.pageCount, pageCurrent, paging.showPagenum), pageNums = []
    ...
    Datagrid.prototype.add = function (addLocation, addData, noEdit) {
        ...
        var that = this, options = that.options, keys = options.keys, tools = that.tools, paging = that.paging, trs,
            obj = {}, data = [], addObj, startNumber = (paging.pageCurrent - 1) * paging.pageSize, linenumber
        ...
        if (addLocation === 'first') {
            ...
            if (options.isTree && that.$lastSelect) {
                var child = that.$lastSelect.data('child') || 0, level = that.$lastSelect.data('level'), $lastTr,
                    hasChild = that.$lastSelect.next().hasClass(that.classnames.tr_child)
    ...
    Datagrid.prototype.edit = function ($trs) {
        var that = this, options = that.options, tools = that.tools, type = options.editMode,
            columnModel = that.columnModel, $editTd
        ...
        $editTd.each(function () {
            var $td = $(this), $btns = $td.find('button.bjui-datagrid-btn'), $edit = $btns.first(),
                $update = $edit.next(), $save = $update.next(), $cancel = $save.next(), $delete = $cancel.next()
    ...
    Datagrid.prototype.externalEdit = function (row, editOpts, data) {
        var that = this, options = that.options, editOptions = {}, $tr, data_index, editUrl, type,
            types = ['dialog', 'navtab', 'div']
    ...
    Datagrid.prototype.doEditRow = function (rows, type, isAdd) {
        ...
        if (typeof rows === 'object') {
            ...
            var $myTrs = that.$tbody.find('> tr:not(.' + that.classnames.tr_child + ')'), $editTrs,
                rows = rows.split(',')
    ...
    Datagrid.prototype.doCancelEditRow = function (row) {
        ...
        $trs.each(function () {
            var $tr = $(this),
                $cancelBtn = $tr.find('> td.' + that.classnames.s_edit + ' > button.bjui-datagrid-btn.cancel')
    ...
    Datagrid.prototype.delRows = function (rows) {
        var that = this, options = that.options, keys = options.keys, beforeDelete = options.beforeDelete,
            confirmMsg = options.delConfirm, $trs
        ...
        if (typeof rows === 'object') {
            ...
            var $myTrs = that.$tbody.find('> tr:not(.' + that.classnames.tr_child + ')'), $delTrs,
                rows = rows.split(',')
        ...
        var delEnd = function () {
            $trs.each(function () {
                var $tr = $(this), childUpdate = that.options.childUpdate,
                    $parent = that.$element.data('bjui.datagrid.parent'),
    ...
    Datagrid.prototype.inlineEdit = function ($tr, isAdd, data) {
        ...
        var that = this, options = that.options, tools = that.tools, columnModel = that.columnModel,
            $tds = $tr.find('> td'), tds_length = $tds.length, tr_index = $tr.index(), data_index = tr_index
        ...
        $tds.each(function (i) {
            var $td = $(this), op = columnModel[i], val = op && op.name && data[op.name], html = $td.html(),
                input = that.inputs[i], $input
            var onChange = function ($el, $td, val) {
                var changeData = $tr.data(that.datanames.changeData), jsontype = op.jsontype,
                    defaultVal = (typeof op.defaultVal === 'undefined') ? null : op.defaultVal
    ...
    Datagrid.prototype.saveAll = function ($trs) {
        var that = this, options = that.options, keys = options.keys, callback = options.editCallback, $tr, data,
            data_index, datas = [], changeData, tempData, postData = [], returnData = [], len = $trs && $trs.length
    ...
    Datagrid.prototype.updateEdit = function ($tr, $btn) {
        var that = this, options = that.options, keys = options.keys, callback = options.editCallback, data, datas = [],
            changeData, tempData, postData = [], returnData, data_index = $tr.index()
        ...
        $trs.each(function () {
            ...
            if ($tr.hasClass(that.classnames.tr_edit)) {
                $tr
                    ...
                    var $td = $(this), td_index = $td.index(), model = columnModel[td_index],
                        html = $td.data(that.datanames.td_html)
    ...
    Datagrid.prototype.inlineEditComplete = function ($tr, trData, $btn) {
        var that = this, columnModel = that.columnModel, tr_index = $tr.index(), $tds = $tr.find('> td'),
            hasLinenumber = false, $lockTr = that.$lockTbody && that.$lockTbody.find('> tr:eq(' + tr_index + ')')
        ...
        if ($tr.hasClass(that.classnames.tr_add)) {
            ...
            // tree
            if (that.options.isTree) {
                var $parentTr = trData[that.options.keys.treePTr], parentData = trData[that.options.keys.treePData],
                    keys = that.options.treeOptions.keys, $treePTd
    ...
    Datagrid.prototype.dialogEdit = function ($tr, isAdd, data) {
        ...
        var that = this, options = that.options, tools = that.tools, columnModel = that.columnModel,
            tr_index = $tr.index(), data_index = tr_index, $dialog, $form, html = '', title
        ...
        var onChange = function ($tr, $form, $el, model) {
            var changeData = $tr.data(that.datanames.changeData), jsontype = model.jsontype,
                defaultVal = (typeof model.defaultVal === 'undefined') ? null : model.defaultVal
        ...
        var onLoad = function ($dialog) {
            ...
            var createForm = function (data, $form, $tr) {
                ...
                $.each(columnModel, function (i, n) {
                    ...
                    var input = that.inputs[i], $input, $p = $('<p></p>'), id = 'datagrid-dialog-edit-column-' + i,
                        val = data[n.name]
    ...
    Datagrid.prototype.dialogEditComplete = function ($tr, trData) {
        var that = this, columnModel = that.columnModel, tr_index = $tr.index(), $tds = $tr.find('> td'),
            hasLinenumber = false, $trs = that.$tbody.find('> tr'),
            $lockTr = that.$lockTbody && that.$lockTbody.find('> tr:eq(' + tr_index + ')')
        var tdTemplate = that.options.tdTemplate, tempData = $.extend({}, trData),
            treeOptions = that.options.treeOptions, treePData, treePTr, treePTd
...
+function ($) {
    ...
    var group, suffix, pk, include, exclude, append, oldData, beforeSelect, onSelect, afterSelect, $box,
        $currentFindGrid
    ...
    FindGrid.empty = function () {
        ...
        var that = this, includes = include.split(','), $inputs = $box.find(':text, :input:hidden, textarea, select'),
            includeKeys = {}
    ...
    FindGrid.setMult = function (gridId) {
        ...
        if (datas && datas.length) {
            var $inputs = $box.find('input:text, input:hidden, textarea, select'), fieldKey, includeKeys = {},
                okObj = {}, excludeKeys = [], v
    ...
    FindGrid.prototype.addBtn = function () {
        ...
        if (!this.$findBtn && !$element.parent().hasClass('wrap_bjui_btn_box')) {
            ...
            this.$findBtn.on('selectstart', function () {
                return false
            })
        }
    }
    ...
    FindGrid.prototype.open = function ($obj) {
        var that = this, options = this.options, dialogOptions = options.dialogOptions,
            gridOptions = options.gridOptions, timestamp = (new Date().getTime())
...
+function ($) {
    ...
    Suggest.setSingle = function (datas) {
        ...
        $.each(keys, function (k, v) {
            var name = that.getField(k),
                $input = $box.find('input[name="' + name + '"], select[name="' + name + '"], textarea[name="' + name + '"]'),
                arr = v ? v.split('/') : ['#' + k + '#'], item, value, label
...
+function ($) {
    ...
    $(document).on(BJUI.eventType.initUI, function (e) {
        ...
        $box.find('textarea').each(function () {
            var $element = $(this).addClass('form-control'), cols = $element.attr('cols') || 0, width = cols * 10,
